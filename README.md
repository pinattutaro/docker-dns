...
- **対象OS**: Ubuntu 22.04 LTS (他のLinuxディストリビューションでも可能)
- **実行環境**: ローカルネットワーク上のサーバ、VM、またはクラウドインスタンス
- **利用ツール**:
    - `docker`
    - `docker-compose` (v2)
- **ネットワーク条件**:
...
### 4.1. 事前準備 (DockerおよびDocker Composeのインストール)
...
2.  **`docker-compose.yml`の作成**
    `vi` や `nano` などのエディタで `docker-compose.yml` ファイルを新規作成し、以下の内容を貼り付けること。
...
- **Pi-holeには接続できているが、広告がブロックされない。**
    ...
    - **対処**: <br>
          1. Client側のネットワーク設定からの代替DNSを空欄にし、IPv6を無効にする。　<br>
          2. Client側の`コントロールパネル > ネットワークとインターネット > ネットワークと共有センター > アダプターの設定の変更`ですべての接続のプロパティから、「インターネット　プロトコル　バージョン6(TCP/IPv6)」のチェックを外す。<br>
